...
```
Here, the aforementioned commands show that ```./lpg``` is the planner executable as mentioned above; **-o** is operator file name i.e. domain file name ```warehousetaskplanner.pddl```; **-f** is problem file name ```warehousetaskplannerproblem1/2/3/4.pddl``` whereas, 1, 2, 3, & 4 are the number of problem files; and last but not the least **-n** is the number of solution i.e. 1 in this execution.  
